...
class ProjectDataset(dataset.base.BaseDataset):
    ...
    def _preprocess(self, project_dir):
        ...
        for line in all_function_list_str.split("\n"):
            ...
            func_body = dataset.utils.function_purification(func_body)
            if func_body == "":
                continue
            # ConstructPath
    ...
    def __init__(self, project_dir: str, seed=20231031, no_cache=False, rebuild=False, skip_loc_threshold=False):
        """
    ...
    def get_funcs(self, size=-1, **kwargs) -> List[str]:
        ...
        if size != -1:
            ...
            func_path_list = rng.sample(dataset.utils.abs_listdir(self.cache_dir),
                                        min(size, len(dataset.utils.abs_listdir(self.cache_dir))))
        else:
